...
const ProjectPreview = () => {
  ...
  return (
    <div className="projectDetails">
      <div className="left">
        <img src={data?.websiteDesign} alt="" />
      </div>
      <div className="right">
        ...
        <div className="projectTechnology">
          ...
          <div className="icons">
            ...
            {data?.isWordpress ? <IoLogoWordpress className="techName wordpress" /> : null}
            {data?.isNode ? <IoLogoNodejs className="techName node" /> : null}
            {data?.isMongoDb ? <SiMongodb className="techName mongodb" /> : null}
          </div>
        </div>
        <div className="links">
          <h2>Project Links:</h2>
          <div className="icons">
          {data?.isFigmaLink ? <Link to={data?.figmaLink}><FaFigma className="techName figma" /></Link> : null}
          {data?.isWebsiteLink ? <Link to={data?.websiteLink}><RiGlobalLine className="techName website" /></Link> : null}
          </div>
        </div>
      </div>
    </div>
